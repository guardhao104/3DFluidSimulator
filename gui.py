...
class SimulationGUI(object):
    ...
    def run(self):
        while self.window.running:
            ...
            # self.camera.track_user_inputs(self.window, movement_speed=0.02, hold_key=ti.ui.RMB)
    ...
    @ti.func
    def compute_screen_pos(self, p):
        # 使用ti.cast确保整数类型，以匹配屏幕坐标的要求
        screen_x = ti.cast(p.x * self.res_x, ti.i32)
        screen_y = ti.cast(p.y * self.res_y, ti.i32)
        return ti.Vector([screen_x, screen_y])
